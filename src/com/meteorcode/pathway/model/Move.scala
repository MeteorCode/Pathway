...
/**
 * An [[com.meteorcode.pathway.model.Event]] that represents an [[com.meteorcode.pathway.model.Entity]]'s attempt to
 * move from one [[com.meteorcode.pathway.model.Tile]] to another.
 *
 * ==How to Use==
 *
 * A Move provides the following [[com.meteorcode.pathway.model.Payload]] when inspected by a
 * [[com.meteorcode.pathway.model.Property]]:
 *  - `"who": Entity` - the [[com.meteorcode.pathway.model.Entity]] that is attempting the move.
 *  - `"to": Tile ` - the [[com.meteorcode.pathway.model.Tile]] that `"who"` wants to move to.
 *  - `"from": Tile ` - the [[com.meteorcode.pathway.model.Tile]] that `"who"` currently occupies
 *  - `"location": Tile ` - the [[com.meteorcode.pathway.model.Tile]] that originated this Event (should be the same as
 *  `"from"`)
 *
 * Note that when a Move evaluates itself, it will check if the target tile is
 * [[com.meteorcode.pathway.model.Tile.occupied]]. If the tile is occupied, the Move will
 * [[com.meteorcode.pathway.model.Event.invalidate]] itself. This shouldn't make a difference, as the
 * [[com.meteorcode.pathway.model.Context.pump]]ing [[com.meteorcode.pathway.model.Context]] has already
 * [[scala.collection.mutable.Stack.pop]]ped it off the `EventStack`, and will just quietly not take place. However,
 ...
 *
 *  1. Add [[com.meteorcode.pathway.model.Property]]s to the game that watch for [[com.meteorcode.pathway.model.Move]]
 *  events that match some criterion,  and invalidate them if they are illegal, or patch their payload with some
 *  additional data. This is probably the Right Thing (I might even go as far as to refer to it as "idiomatic Pathway".
 *  Note that [[com.meteorcode.pathway.model.Move]] stores its data (original tile, target tile, entity that's moving)
 *  in a Payload rather than as fields in the Move class specifically so that you can interact with Moves like any other
 *
 *  2. Extend [[com.meteorcode.pathway.model.Move]] with different subclasses representing the different types of Move
 ...
 *  our [[com.meteorcode.pathway.model.Grid]]/[[com.meteorcode.pathway.model.Tile]] system at all. Otherwise, I'd
